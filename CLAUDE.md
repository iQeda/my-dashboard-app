...
- `lib.rs` - Tauri Builder にコマンドとプラグイン (dialog, log, global-shortcut, updater, process, autostart) を登録。`register_shortcut` / `unregister_all_shortcuts` コマンド。WKWebView の `allowsBackForwardNavigationGestures` を `objc2` クレートで有効化

### CI/CD (`.github/workflows/`)

- `ci.yml` - PR・push 時に `tsc -b` + ESLint + vitest + Rust `cargo check` / `cargo test`（concurrency で古い run を自動キャンセル）
...
- `HOMEBREW_TAP_TOKEN` - `iQeda/homebrew-tap` への push 用 PAT（Fine-grained PAT、Contents: Read and write。**有効期限切れに注意**）

#### リリースのトラブルシュート

- **Homebrew Cask 更新だけ失敗する**（`Invalid username or token` / exit 128）: `HOMEBREW_TAP_TOKEN` の失効。ビルドと GitHub Release 作成は成功しているので、タグの切り直しは不要。PAT を再発行して Secret を差し替えた上で、当該リリース分は手動で cask を更新する:
  ```bash
  gh release download vX.Y.Z --repo iQeda/my-dashboard-app --pattern "MyDashboard.app.tar.gz" -D /tmp
  shasum -a 256 /tmp/MyDashboard.app.tar.gz   # この sha256 を使う
  git clone git@github.com:iQeda/homebrew-tap.git /tmp/homebrew-tap
  # /tmp/homebrew-tap/Casks/mydashboard.rb の version と sha256 を書き換え
  # （テンプレートは release.yml の "Update Homebrew Cask" ステップと同一に保つ）
  cd /tmp/homebrew-tap && git commit -am "Update MyDashboard cask to vX.Y.Z" && git push
  ```
- リリースのやり直しが必要な場合は GitHub Release とタグを削除してからタグを push し直す（`gh release delete vX.Y.Z --cleanup-tag`）

### Capabilities (`src-tauri/capabilities/default.json`)

- `core:default`, `dialog:default`（open/save を包含）
- `updater:default` - アプリ内自動更新
- `process:default` - アプリ再起動 (`relaunch`)
- `autostart:default` - ログイン時自動起動
- グローバルショートカットは Rust 側 API のみ使用のため `global-shortcut:*` permission は不要

### React Frontend (`src/`)

- `App.tsx` - `App`(useConfig + ConfigContext.Provider + I18nProvider) + `AppContent`(メインUI) の2層構造。**`useConfig()` は App で1回だけ呼び、`ConfigContext` で共有する（二重インスタンス化は configRef 分裂によるデータ損失を招くため禁止）**
- カスタムフック群（App.tsx から分解済み）:
  - `useNavigation` - pageView + navigateTo（history.pushState / popstate）
  - `useViewPrefs` - cardSize / viewMode / sidebarWidth のローカルミラー
  - `useSidebarResize` - サイドバー幅の Pointer Events リサイズ
  - `useLauncher` - launchAndRecord + openAll（起動 + アクセス記録の一元化）
  - `useAppShortcuts` - 集約キーボードハンドラ（単一 capture ハンドラ + ref 参照）
  - `useDismiss` / `usePointerReorder` / `useUpdater` - UI 共有フック
- `src/utils/` - 純粋ロジックの単一ソース:
  - `groupItems.ts` - カテゴリグループ順序（App のフォーカス順と Dashboard の描画順は必ずこれを使う）
  - `labels.ts` - slugify / uniqueId（while-suffix）/ compareLabels / sortByLabel
  - `pinned.ts` - getOrderedPinnedEntries（Sidebar / DashboardOverview 共通のピン順序解決）
  - `recent.ts` - resolveRecentItems
- UI プリミティブ: `MenuSurface`（右クリックメニュー共通: portal + clamp + dismiss）、`ModalShell`（モーダルオーバーレイ共通: SKILL.md #5 パターン）、`icons.tsx`、`item-parts.tsx`（ItemCard/ItemRow 共有部品）、`CategoryTagCard`
...
- `useConfig` hook - 設定の CRUD。`configRef` パターンで最新 state を参照
  ...
  - updateViewPrefs (viewMode, cardSize, sidebarWidth, sidebarCategoriesOpen, sidebarTagsOpen, combinedFilter, multiTagMode, pinnedOrder, dismissedUpdateVersion 永続化。ViewPrefs 型)
  - updateLocale (言語設定)
...
Load:   Rust load_config_from_file → backups/ へ自動バックアップ後 config.json を上書き → reload
```
...
- `AppConfig` - items[], tagDefs[], categoryList?, emojiHistory?, viewMode?, cardSize?, sidebarWidth?, locale? (Locale 型), recentAccess?, globalShortcut?, sidebarCategoriesOpen?, sidebarTagsOpen?, combinedFilter?, multiTagMode?, pinnedOrder?, dismissedUpdateVersion?
...
- **excludeFromOpenAll フラグ**: アイテム編集画面でトグル。Open All（ボタン / ⌘O）の対象から除外
...
- **config を上書きする機能を作る場合、必ず `backups/` サブディレクトリにタイムスタンプ付きバックアップを自動作成する**（現在 `load_config_from_file` に実装済み。プロファイル切替機能 `switch_config` は削除済み）
- **バックアップファイルは config ディレクトリ直下に置かない。** `backups/` サブディレクトリに隔離する
- **プロファイル切替のような破壊的操作は、設計段階で「元に戻せるか」を検証してから実装する**

### バックアップの定期作成

現在の実装:
- `load_config_from_file` 実行時に `backups/config-{unix_timestamp}.json` として自動バックアップ
...
- 全 UI テキストは `t()` 関数経由（ハードコード禁止）。例外: App.tsx の Loading/Error 表示のみ I18nProvider マウント前のため英語固定
